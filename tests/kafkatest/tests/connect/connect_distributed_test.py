...
class ConnectDistributedTest(Test):
    ...
    def test_bounce(self, clean, connect_protocol):
        ...
        # This timeout should be longer than scheduled.rebalance.max.delay.ms
        wait_until(lambda: self.is_running(self.source), timeout_sec=90,
                   err_msg="Failed to see connector transition to the RUNNING state")
        time.sleep(15)
        self.source.stop()
        ...
        # This timeout should be longer than scheduled.rebalance.max.delay.ms
        wait_until(lambda: self.is_running(self.sink), timeout_sec=90,
                   err_msg="Failed to see connector transition to the RUNNING state")
        time.sleep(15)
        self.sink.stop()
        self.cc.stop()
